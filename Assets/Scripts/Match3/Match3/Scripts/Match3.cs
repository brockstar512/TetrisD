...
public class Match3 : MonoBehaviour
{
    ...
    void InstantiateBoard()
    {
        for (int x = 0; x < width; x++)
        {
            for (int y = 0; y < height; y++)
            {
                ...
                Debug.Log($"Here is val {val}");//val keeps saying 7

                Debug.Log($"Here is cells {pieces[val - 1]}");
    ...
    void KillPiece(Point p)
    {
        ...
        for (int i = 0; i < killed.Count; i++)
            if (!killed[i].falling) available.Add(killed[i]);//if you havent already killed the piece add it to killed pice
    ...
    {
        int val = 1;
        val = (random.Next(0, 100) / (100 / pieces.Length));//val = (random.Next(0, 100) / (100 / pieces.Length)) + 1;
        return val;
    }
